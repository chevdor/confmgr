...
- You create your config specs using a Factory or a YAML file (Using YAML is recommended)
...
For this reason, `confmgr` helps you keeps things nice, tidy and collision-free thanks to a `PREFIX` and `MODULE`. Those are 2 strings, usually in CAPS. Any of your configuation value will be prepented with `PREFIX_MODULE_`.
...
[source, typescript]
----
include::src/types.ts[tag=ConfigItemOptions]
----
